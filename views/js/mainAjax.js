$(function(){
	$.ajax({
		url: '/HelpHomework/src/getName.php',
		type: 'GET',
		dataType: 'html',
	...
	$(document).on('click','span.com-icon',function() {
	    ...
	    if($com.css('display')=='none'){
			$com.slideDown();
			if(loaded == '0'){
				$.post('/HelpHomeWork/src/getComments.php',{
			    	pubtime: pubtime
	...
	$.getJSON('/HelpHomework/src/getAllContents.php', function(data,status) {
		if(status == 'success'){
			var jsonData = eval(data);
	...
	$(document).on('click','input[type="button"]',function(){
		...
		if(cont!='' && $(this)[0].value == '发布'){
			$.post('/HelpHomework/src/postContent.php',{
				content: cont
			},function(data,status){
				if(status == 'success'){
			...
			$.post('/HelpHomework/src/postComment.php',{
				comment: cont,
				pubtime: pubtime
			},function(data,status){
	...
	$(document).on('click','span.goods',function(){
		...
		if(time!=''){
			$.post('/HelpHomework/src/good.php',{
				pubtime: time
			},function(data,status){
				if(status == 'success'){
					if(data == 'suc'){
